#!/bin/bash
# Universal Stealth Locksmith - Hardware Fingerprint Version

echo "--- Step 1: Fingerprinting Hardware ---"
# Detect CPU and Disk Size for a unique ID
...
if [ -z "$CRYPT_NAME" ] || [ -z "$USB_PART" ]; then
    echo "ERROR: Could not find required hardware. Is USB plugged in?"
    exit 1
fi
...
if [ ! -f "$KEYFILE" ]; then
    ...
    echo "New stealth key created."
fi

echo "Please enter LUKS password to register this PC:"
...
/lib/cryptsetup/askpass "Stealth key not found. Enter LUKS Password: "
EOF
sudo chmod +x /usr/local/sbin/usb-unlock

# 5. Final Configuration
echo "--- Step 5: Finalizing Boot Image ---"
...
echo "--- COMPLETE ---"
echo "Stealth key is now active. You can take the USB out immediately after boot."
